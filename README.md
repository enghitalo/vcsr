...
| 12 🚧 | [12](tests/phase_12_runtime_signals_test.v) · [13](tests/phase_13_runtime_ffi_test.v) · [14](tests/phase_14_runtime_engine_test.v) · [15](tests/phase_15_runtime_app_test.v) | the **runtime library** the generated code imports — **in progress**. Done: slice 1 (reactive core `Signal`/`effect` with dynamic dependency tracking + disposal, [signal.v](signal.v)) ✅, slice 2 (`js` FFI substrate `JsValue` get/set/call/new + mock host, [js_ffi.v](js_ffi.v)) ✅, slice 3 (Template/bind engine: `instance()` clone → `bind_*` reactive slots → `View`, [runtime/runtime.v](runtime/runtime.v)) ✅, slice 4 (app runtime: `Component`/`new_app`/`render`/`mount`/`unmount`/`document`, cross-component state via shared signals, [runtime/app.v](runtime/app.v)) ✅. Next: make the generated `counter` compile end-to-end against the runtime. This closes the loop from a triplet to a running app. |
